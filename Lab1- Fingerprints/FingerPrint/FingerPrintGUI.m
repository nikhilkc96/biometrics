...
%      FINGERPRINTGUI, by itself, creates a new FINGERPRINTGUI or raises loadthe existing
%      singleton*.
%
...
ind=intersect(find(~isnan(CentroidSepX)),find(~isnan(CentroidSepY)));
CentroidSepX=CentroidSepX(ind);
CentroidSepY=CentroidSepY(ind);
dySep=dySep(ind,:);
dxSep=dxSep(ind,:);
OrientationSep=OrientationSep(ind,:);
...
%added to fix - Sim1
[hsz,wsz,chsz]=size(OriginalImage);
if (chsz==1)
    OriginalImage2=zeros(hsz,wsz,3);
    OriginalImage2(:,:,1)=OriginalImage;
    OriginalImage2(:,:,2)=OriginalImage;
    OriginalImage2(:,:,3)=OriginalImage;
    OriginalImage2=uint8(OriginalImage2);
